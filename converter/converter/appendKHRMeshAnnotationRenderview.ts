...
export function appendKHRMeshAnnotation(gltf: GLTF.IGLTF): void {
  console.warn(
    'First-person mesh annotations in VRM are associated with nodes, while KHR_mesh_annotation is associated with meshes. ' +
      ...
      `In this implementation, we will try assuming meshes that should be thirdPersonOnly by simply checking the mesh name (${
        firstPersonKeywords.map((keyword) => `"${keyword}"`).join(', ')
      }). `,
  );
  ...
  for (const mesh of gltf.meshes ?? []) {
    let renderVisibility: KHRMeshAnnotationRenderviewRenderVisibility =
      'always';
    ...
    for (const keyword of firstPersonKeywords) {
      if (mesh.name?.toLowerCase().includes(keyword)) {
        ...
        logVerbose(
          `KHR_mesh_annotation_renderview: Mesh "${mesh.name}" has keyword "${keyword}"`,
        );
        break;
      }
    }
    ...
    logVerbose(
      `KHR_mesh_annotation_renderview: Mesh "${mesh.name}" set to "${renderVisibility}"`,
    );
    logVerbose(
      `KHR_mesh_annotation_renderview: Adding extension to ${
        mesh.primitives?.length ?? 0
      } primitive(s) of mesh "${mesh.name}"`,
    );
